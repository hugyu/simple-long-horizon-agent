...
> 阅读入口：先看 [00. 用一个真实运行看懂 Agent](00-running-example.md)，再按专题理解设计。
>
> 当前状态：本目录包含一篇贯穿式运行案例和 19 篇专题文档；后续以源码、测试和评审结果持续校正。
>
...
### 00. [`00-running-example.md`](00-running-example.md)——用一个真实运行看懂 Agent

这是推荐的第一篇。它使用仓库自带 Fake Provider 和 Bash Demo，展示：

- `Agent.run()` 返回惰性 Event Stream 的含义；
- 一次任务实际产生的 4 条 Message 和事件顺序；
- `ToolCallBlock.id` 与 `ToolResultBlock.tool_call_id` 如何建立因果关系；
- `ContextView`、`LLMRequest` 和 Provider Adapter 的边界；
- 失败结果、停止原因以及 Compression、Recall、Skills、Memory 在长任务中的位置。

它不是新的协议说明，而是后续所有专题共享的事实样本。
...
先用 00 建立运行直觉，再用 01～04 解释问题、模块和循环。这五篇构成最小完整认知路径。
...
1. **本篇解决的问题**：为什么值得单独讨论。
2. **先看一个具体输入**：任务、配置和可观察输出。
3. **数据快照**：Message、Event、State 或请求字段如何变化。
4. **控制流**：谁在什么时候调用谁，何时进入下一轮。
5. **职责与边界**：负责什么，不负责什么。
6. **异常与停止**：失败、预算、恢复和降级行为。
7. **设计理由与取舍**：为什么采用当前方案。
8. **源码与测试证据**：事实对应的实现和验证位置。
9. **理解检查**：读者能否用同一案例复述数据流。
